 ...
 if ( !empty($_POST)) {
    ...
    if (empty($id_centre)) {
     ...
     if ($valid) {
         ...
         $dbh = "INSERT INTO profil (nom_profil,prenom_profil,promotion_profil, identifiant_profil, password_profil, id_centre) values(?, ?, ?, ?, ?, ?)";
         $q = $sql->prepare($dbh);
         $q->execute(array($nom_profil,$prenom_profil,$promotion_profil, $identifiant_profil, $password_profil, $id_centre));
